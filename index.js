...
async function getImdbId(title) {
    const response = await fetch(`https://www.omdbapi.com/?apikey=4044abd7&s=${title}`)
...
async function getContentInfo(IDs) {
    const contentInfo = await Promise.all(
        IDs.map(async id => {
            const response = await fetch(`https://www.omdbapi.com/?apikey=4044abd7&i=${id}`)
            const data = await response.json()
            return data
        })
    )
...
function renderSearchResults(contentInfo, tagToRender) {  
    ...
    tagToRender.innerHTML = contentInfo.map(content => {
        ...
        // Check if title is already on watchlist and decide which button to render
